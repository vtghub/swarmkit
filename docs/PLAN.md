...
**MCP integration**: `mcp_server/server.py` uses the official `mcp` Python SDK's `FastMCP` for protocol handling, but each tool handler delegates its actual execution to the Rust native module for the hot path (arg validation, subprocess dispatch, result serialization) — genuinely-functional tools (`spawn_agent`, `get_task_status`, `query_memory`, `list_agents`) each backed by a real daemon RPC — stdio transport for local use, Streamable HTTP for networked use (reused later by federation). `swarmkit` is also an MCP *client*: `mcp_server/client_tools.py` uses the Anthropic SDK's MCP helpers (`anthropic.lib.tools.mcp`) to pull tools from third-party MCP servers directly into an agent's tool loop — this is the integration point for `vtghub/mcp-native-core` (added for faster search/parsing); until that repo's real tool schemas are inspected, treat it as just another upstream MCP server consumed through this same path, with no swarmkit-side special-casing needed. Verify exact `mcp` SDK class/decorator names against the upstream python-sdk repo before implementing Phase 4.
...
swarmkit/
  ...
  LICENSE                     # MIT
  crates/
    swarmkit-core/           # lib crate: worker pool, task queue, sandboxed subprocess exec
      src/{worker_pool.rs, taskqueue.rs, sandbox.rs, vectors.rs, mcp_tool_exec.rs}
    swarmkit-py/              # PyO3 binding crate -> builds swarmkit._native
      src/lib.rs
  src/swarmkit/
    cli/             main.py, daemon_client.py
    daemon/          server.py, supervisor.py
    ...
    mcp_server/      server.py, client_tools.py                    # tools.py's hot path delegates to swarmkit._native.mcp_tool_exec
...
- **Phase 4** — MCP server exposing real tools, with hot-path execution in Rust. Done when: an external MCP client can trigger a real daemon-scheduled task; swarmkit consumes both a generic external MCP server and (once inspected) `vtghub/mcp-native-core`'s search/parse tools.
